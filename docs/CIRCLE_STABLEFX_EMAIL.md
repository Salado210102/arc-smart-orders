...
**What we built (live on Arc mainnet, open source, MIT):**
...
- StableFX is **permissioned** (RFQ, institutional). Pending access, the order engine's swap leg stays
  un-wired on mainnet (no public AMM yet); the graduation venue is a separate open question (see
  `MAINNET_RUNBOOK.md` §2.3).
